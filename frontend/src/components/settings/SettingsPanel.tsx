...
export function SettingsPanel() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (braveKey) data.brave_search_api_key = braveKey;
      data.ollama_base_url = ollamaUrl;
      data.default_model = selectedModel;
      await api.updateSettings(data);
      setMessage('Settings saved!');
      setOpenaiKey(''); setAnthropicKey(''); setGeminiKey(''); setBraveKey('');
                          ...
                          {activeTab === 'api_keys' && (
                            <div className="space-y-5">
                              {[
                                ...
                                { label: 'Brave Search API Key', key: braveKey, setter: setBraveKey, set: settings?.brave_search_api_key_set, placeholder: 'BS...' },
